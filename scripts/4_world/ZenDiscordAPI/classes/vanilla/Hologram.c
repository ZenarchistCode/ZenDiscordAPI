modded class Hologram
{
	...
	override void RefreshVisual()
    {
        ...
        if (m_Parent && m_Parent.IsInherited(Zen_RaidAlarmRadarKit) && m_Player.IsPlacingLocal())
        {
            ...
            if (zenkitbox != NULL)
            {
                zenkitbox.SetZenHologrammed(m_Projection != NULL);
            }
        }
    }
	...
	void ~Hologram()
    {
        if (GetGame())
        {
            if (m_Parent && m_Parent.IsInherited(Zen_RaidAlarmRadarKit))
            {
                ...
                if (zenkitbox != NULL)
                {
                    zenkitbox.SetZenHologrammed(false);
                }
            }
        }
    }
	...
	{
		EntityAI superEntityAI = super.PlaceEntity(entity_for_placing);

		if (superEntityAI == NULL)
		{
			return NULL;
		}
