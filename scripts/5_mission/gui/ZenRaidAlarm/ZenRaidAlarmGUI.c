class ZenRaidAlarmGUI extends UIScriptedMenu
{
	...
	static const string	LAYOUT_FILE		= "ZenDiscordAPI/data/gui/layouts/RaidAlarm/raid_alarm.layout";
	...
	override bool OnClick(Widget w, int x, int y, int button)
	{
		...
		if (button == MouseState.LEFT)
		{
			...
			if (w == m_OkBtn)
			{
				webhook = m_Webhook1.GetText();
				if (!ValidateWebhook(webhook))
				{
					m_Webhook1.SetText("");
				}

				webhook = m_Webhook2.GetText();	
				if (!ValidateWebhook(webhook))
				{
					m_Webhook2.SetText("");
				}

				webhook = m_Webhook3.GetText();
				if (!ValidateWebhook(webhook))
				{
					m_Webhook3.SetText("");
				}
			...
			if (w == m_PasteBtn1)
			{
				GetGame().CopyFromClipboard(webhook);

				if (ValidateWebhook(webhook))
					m_Webhook1.SetText(webhook);
